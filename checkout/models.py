...
class CheckoutOrder(models.Model):
    """
    Checkout oreder model for customer to enter their
    details for order to be delivered
    """
    # Found other way to use UUID here:
    # https://stackoverflow.com/questions/32528224/how-to-use-uuid
    order_id = models.UUIDField(default=uuid.uuid4, max_length=32,
...
class CheckoutLineItem(models.Model):
    ...
    order = models.ForeignKey(CheckoutOrder, null=False, blank=False,
    ...
    def save(self, *args, **kwargs):
        """
        Override the original save method, set item total
    ...
    def __str__(self):
        return f'{self.size} {self.product.display_name} on order\
                 {self.order.order_id}'
